(() => {
  ...
  function createState() {
    ...
    const created={
      version:36, role:setup.role, origin:setup.origin, person, clubId:club.id, date:START_DATE, season:2026, view:"home",
  ...
  function playerExternalMarketInterest(save,player) {
    const club=clubById(save.clubId),average=averageRating(player)||6.35,contractUrgency=Number(player.contract?.endSeason||save.season+3)<=save.season+1?12:0,performance=Math.max(0,average-6.35)*14+Math.min(10,Number(player.appearances||0)*.34),mobility=Math.max(0,Number(player.potential||player.overall)-Number(player.overall||0))*.7;
    return clamp(Math.round(32+Math.max(-7,Number(player.overall||0)-Number(club.prestige||70))*3.1+performance+mobility+contractUrgency),26,88);
  }
  ...
  function createUserClubExternalOffer(save,date=save.date) {
    ...
    for(const player of save.squad||[]){
      ...
      for(const buyer of CLUBS){
        ...
        const needs=aiSquadNeeds(save,buyer.id),need=needs.needs[role],budget=Number(market.budgets[buyer.id]||buyer.budget||0),fee=transferFee(player,buyer,seller,market.sequence,save);
        if(!need||budget<fee||player.overall>buyer.prestige+9||player.overall<buyer.prestige-20)continue;
        const performance=Math.max(0,(averageRating(player)||6.35)-6.35)*8+Math.min(7,Number(player.appearances||0)*.24),appeal=need.score*2+Math.max(0,player.overall-need.average)*1.8+Math.max(0,player.potential-player.overall)*.45+performance+depth*2+(buyer.prestige-seller.prestige)*.2+(stableScoutingUnit(`${date}|club-offer|${buyer.id}|${player.id}`)-.5)*8;
