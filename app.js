(() => {
  ...
  function createState() {
    ...
    const created={
      version:35, role:setup.role, origin:setup.origin, person, clubId:club.id, date:START_DATE, season:2026, view:"home",
  ...
  function createPlayerCareer(player,club) {
    ...
    return {
      ...
      objectiveProgress:{appearances:0,ratings:0,goals:0,assists:0},contractStance:"留队竞争",renewalNegotiation:null,renewalLastEvaluatedDate:null,transferOffer:null,transferOfferHistory:[]
    };
  }
  ...
  function renderPlayerHome() {
    ...
    return `<div class="player-command-heading"><div><span class="eyebrow">${state.season}/${String(state.season+1).slice(2)} · 球员生涯</span><h2>${esc(player.name)}</h2><p>${clubNameLink(club.name)} · ${playerRoleLabel(player.position)} · ${career.status}</p></div><div class="player-heading-metrics"><div class="player-overall"><span>当前能力</span><strong>${player.overall}</strong></div><div class="player-fitness-summary ${fitnessTone}"><span>体力</span><strong>${fitness}%</strong><i><b style="width:${fitness}%"></b></i><small>${fitnessLabel}</small></div></div></div>
      ...
      ${transferOffer?`<section class="career-alert renewal"><div>${icon("repeat-2")}<span><strong>${esc(clubById(transferOffer.toId).name)} 送来正式转会报价</strong><small>转会费 ${money(transferOffer.fee)} · 周薪 €${Math.round(transferOffer.contract.weeklyWage)}K · ${transferOffer.contract.years} 年 · ${esc(transferOffer.contract.role)} · ${formatDate(transferOffer.deadlineDate,false)} 前回复</small></span></div><button class="btn btn-primary" data-open-player-transfer-offer="${transferOffer.id}">查看报价</button></section>`:""}
  ...
  function createPlayerTransferRequestOffer(save,date=save.date) {
    if(save.role!=="player")return null;const player=(save.squad||[]).find(item=>item.id===save.controlledId),career=ensurePlayerCareer(save),market=ensureTransferMarket(save);if(!player||player.transferRequestStatus!=="submitted"||career.transferOffer?.status==="active")return null;
    ...
    const candidates=CLUBS.filter(club=>club.id!==seller.id&&!previousClubs.has(club.id)).map(buyer=>{
      const budget=Number(market.budgets[buyer.id]||buyer.budget||0),needs=aiSquadNeeds(save,buyer.id),need=needs.needs[role],fee=Number((transferFee(player,buyer,seller,market.sequence,save)*.84).toFixed(1)),abilityFit=Number(player.overall||0)-Number(need?.average||buyer.prestige-12),prestigeFit=12-Math.abs((buyer.prestige-4)-Number(player.overall||0)),interest=playerTransferRequestChance(save,player);
    ...
    const offer={id:`player-offer-${save.season}-${player.id}-${++market.sequence}`,status:"active",createdDate:date,deadlineDate:addDays(date,12),windowKey,playerId:player.id,playerName:player.name,fromId:seller.id,toId:target.buyer.id,fee:target.fee,confidence:target.interest,reason:"球员已提交正式转会申请，买方加速推进谈判",contract};career.transferOffer=offer;player.transferRequestStatus="offer-received";
    save.transferRequestsLog.unshift({id:offer.id,date,type:"player-offer",playerId:player.id,playerName:player.name,fromId:seller.id,toId:target.buyer.id,status:"active"});addNotification({title:`正式报价：${target.buyer.name} 希望签下 ${player.name}`,type:"transfer",date,detail:"提交转会申请后，经纪人已将你的离队意愿传达给市场。对方送来正式报价，最终去留由你决定。",facts:[`转会费：${money(target.fee)}`,`周薪：€${wage}K · ${years} 年`,`队内角色：${contract.role}`]});reportTransferStory(save,`${target.buyer.name} 向 ${player.name} 提交正式报价`,`${player.name} 提交转会申请后，${target.buyer.name} 加快了谈判进程。预计转会费为 ${money(target.fee)}。`,date,true);return offer;
  }

  function completeControlledPlayerTransfer(offer) {
    const player=controlledPlayer(),market=ensureTransferMarket(state),seller=clubById(state.clubId),buyer=clubById(offer.toId);if(!player||!buyer)return false;
    const careerSegment=transferCareerSegment(state,player,seller,state.date,true),careerHistory=[...(player.careerStats||[])],contract=transferContract(player,{contract:offer.contract},state.season),record={id:`transfer-${market.season}-${++market.sequence}`,season:state.season,windowKey:offer.windowKey,date:state.date,playerId:player.sourcePlayerId||player.id,playerName:player.name,position:player.position,overall:player.overall,potential:player.potential,fromId:seller.id,toId:buyer.id,fee:offer.fee,reason:"球员正式转会申请获批",contract,careerHistory,careerSegment};
    const budget=Number(market.budgets[buyer.id]||buyer.budget||0);if(budget<offer.fee)return false;market.budgets[buyer.id]=Number(Math.max(0,budget-offer.fee).toFixed(1));market.budgets[seller.id]=Number((Number(market.budgets[seller.id]||0)+offer.fee*.82).toFixed(1));market.clubOverrides[player.sourcePlayerId||player.id]=buyer.id;
    const moved={...player,club:buyer.id,clubId:buyer.id,joinedDate:state.date,careerStats:[...careerHistory,careerSegment],contract,weeklyWage:contract.weeklyWage,wage:contract.weeklyWage,fitness:94,morale:84,form:0,lastRating:null,ratingTotal:0,appearances:0,goals:0,assists:0,injured:0,injury:null,consecutiveStarts:0,lastMatchMinutes:0,lastMatchDate:null,lastSelectionStatus:null,transferRequestStatus:"completed"};["tackles","tacklesWon","interceptions","clearances","blocks","duels","duelsWon","saves","cleanSheets","keyPasses","chancesCreated","successfulDribbles","progressivePasses","recoveries","pressuresWon"].forEach(key=>{moved[key]=0;});delete moved.development;ensurePlayerDevelopment(moved,state.season);initializePlayerMarketValue(moved,state,state.date);
    const newSquad=createSquad(buyer,moved);newSquad.forEach(item=>{ensurePlayerContract(item,state.season);ensurePlayerDevelopment(item,state.season);});const controlledIndex=newSquad.findIndex(item=>item.id==="controlled");newSquad[controlledIndex>=0?controlledIndex:0]=moved;state.clubId=buyer.id;state.squad=newSquad;state.controlledId="controlled";state.schedule=generateSchedule(buyer,state.season,moved).filter(fixture=>fixture.date>=state.date);state.played=0;state.wins=0;state.draws=0;state.losses=0;state.points=0;state.leaguePosition=1;state.competitionProgress={europe:createEuropeanProgress(buyer,state.season),cups:{},international:{}};state.majorLeagueWorld=createMajorLeagueWorld(state.season,buyer.id);simulateMajorLeagueWorld(state,state.date);state.majorLeagueId=buyer.league;state.funds=Number(market.budgets[buyer.id]||buyer.budget||0);state.activeMatch=null;
    market.records.unshift(record);state.transferHistory=Array.isArray(state.transferHistory)?state.transferHistory:[];state.transferHistory.unshift(record);state.transferHistory=state.transferHistory.filter((item,index,list)=>list.findIndex(other=>other.id===item.id)===index).slice(0,500);const career=ensurePlayerCareer();career.transferOffer={...offer,status:"accepted",completedDate:state.date};career.transferOfferHistory.unshift(career.transferOffer);career.transferOfferHistory=career.transferOfferHistory.slice(0,20);career.trust=55;career.chemistry=54;career.relationships={...career.relationships,coach:52,teammates:52,captain:50};career.status="新环境适应";career.contractStance="已转会";state.transferRequestsLog.forEach(item=>{if(item.id===offer.id)item.status="completed";});addNotification({title:`官方：${moved.name} 加盟 ${buyer.name}`,type:"transfer",date:state.date,detail:"转会申请已获批。旧俱乐部阶段的所有数据已经归档，新俱乐部生涯从今天开始。",facts:[`${seller.name} → ${buyer.name}`,`转会费：${money(offer.fee)}`,`新合同至 ${contract.endSeason} 年 · ${contract.role}`]});state.media.unshift({source:"Transfer Desk",title:`官方：${moved.name} 加盟 ${buyer.name}`,body:`${buyer.name} 以 ${money(offer.fee)} 从 ${seller.name} 签下 ${moved.name}。球员此前已提交正式转会申请。`,date:state.date,type:"transfer"});return true;
  }

  function acceptPlayerTransferOffer(id) {const offer=ensurePlayerCareer()?.transferOffer;if(!offer||offer.id!==id||offer.status!=="active")return;if(!completeControlledPlayerTransfer(offer)){toast("买方预算或报价状态已发生变化");return;}modal=null;saveState();render();toast("转会已经完成，欢迎来到新俱乐部");}

  function rejectPlayerTransferOffer(id) {const career=ensurePlayerCareer(),offer=career?.transferOffer;if(!offer||offer.id!==id||offer.status!=="active")return;offer.status="rejected";offer.rejectedDate=state.date;career.transferOfferHistory.unshift(offer);career.transferOfferHistory=career.transferOfferHistory.slice(0,20);const player=controlledPlayer();player.transferRequestStatus="submitted";state.transferRequestsLog.forEach(item=>{if(item.id===offer.id)item.status="rejected";});addNotification({title:`你拒绝了 ${clubById(offer.toId).name} 的报价`,type:"transfer",date:state.date,detail:"转会申请仍然有效，市场会在之后继续寻找其他合适机会。",facts:[`拒绝报价：${money(offer.fee)}`,`当前合同至 ${player.contract?.endSeason} 年`]});modal=null;saveState();render();toast("已拒绝该报价");}
  ...
  function renderTransfers() {
    ...
    return `<div class="page-heading"><span class="eyebrow">${windowOpen?"转会窗口开放":"转会窗口关闭"}</span><h2>转会中心</h2><p>转会费与个人合同分阶段谈判，报价结构、承诺和回复方式都会消耗对方耐心。</p></div>
      ...
      ${state.role==="player"?`<div class="panel transfer-actions-panel"><div class="panel-header"><h3>个人转会立场</h3><span class="meta">${transferStatus}</span></div><div class="panel-body">${playerOffer?`<section class="career-alert renewal"><div>${icon("file-signature")}<span><strong>${esc(clubById(playerOffer.toId).name)} 已提交正式报价</strong><small>转会费 ${money(playerOffer.fee)} · 周薪 €${Math.round(playerOffer.contract.weeklyWage)}K · ${playerOffer.contract.years} 年 · ${esc(playerOffer.contract.role)} · ${formatDate(playerOffer.deadlineDate,false)} 前答复</small></span></div><button class="btn btn-primary" data-open-player-transfer-offer="${playerOffer.id}">查看报价</button></section>`:controlledPlayer()?.transferRequestStatus==="submitted"?`<div class="data-note">你的离队意愿已经进入市场优先候选池。符合预算、位置需求和实力匹配的俱乐部会优先评估，并以正式报价联系你。</div>`:""}<div class="grid grid-2"><button class="btn" id="request-transfer" ${!windowOpen||["submitted","offer-received"].includes(controlledPlayer()?.transferRequestStatus)?"disabled":""}>${icon("send")}正式提交转会申请</button><button class="btn" id="suggest-signing">${icon("message-square")}向教练建议引援</button></div></div></div>`:`<section class="panel transfer-actions-panel"><div class="panel-header"><h3>挂牌与离队管理</h3><span class="meta">向董事会提交正式名单</span></div><div class="transfer-list-squad">${[...state.squad].sort((a,b)=>a.overall-b.overall).slice(0,8).map(player=>`<div><span><strong>${esc(player.name)}</strong><small>${playerRoleLabel(player.position)} · ${player.overall} · ${money(player.value)}</small></span><button class="btn btn-sm ${player.listed?"btn-danger":""}" data-list-player="${esc(player.id)}">${player.listed?"撤销挂牌":"挂牌出售"}</button></div>`).join("")}</div></section>`}
  ...
  function renderModal() {
    ...
    if (modal.type === "playerTransferOffer") {
      const career=ensurePlayerCareer(),offer=career?.transferOffer;if(!offer||offer.id!==modal.id)return "";const buyer=clubById(offer.toId),seller=clubById(offer.fromId),contract=offer.contract||{},active=offer.status==="active";
      return `<div class="modal-backdrop"><div class="modal negotiation-modal" role="dialog" aria-modal="true"><div class="modal-header"><div><span class="eyebrow">正式转会报价 · ${active?"等待球员决定":"报价已结束"}</span><h2>${esc(buyer.name)}</h2></div><button class="btn btn-icon btn-ghost" data-close-modal aria-label="关闭">${icon("x")}</button></div><div class="negotiation-patience renewal-patience"><div class="renewal-deadline"><span>报价截止</span><strong>${formatDate(offer.deadlineDate,false)}</strong></div><div class="renewal-deadline"><span>市场匹配度</span><strong>${offer.confidence}%</strong></div></div><div class="modal-body negotiation-body"><section class="negotiation-summary"><div><span>转会费</span><strong>${money(offer.fee)}</strong></div><div><span>转会路径</span><strong>${esc(seller.name)} → ${esc(buyer.name)}</strong></div><div><span>当前能力</span><strong>${controlledPlayer()?.overall || "-"}</strong></div><div><span>队内角色</span><strong>${esc(contract.role||"轮换球员")}</strong></div></section><section class="negotiation-form"><h3>个人合同条款</h3><div class="negotiation-fields"><label>周薪<small>千欧元</small><strong>€${Math.round(Number(contract.weeklyWage||0))}K</strong></label><label>签约奖金<small>百万欧元</small><strong>${money(contract.signingBonus||0)}</strong></label><label>出场津贴<small>千欧元</small><strong>€${Math.round(Number(contract.appearanceFee||0))}K</strong></label><label>解约金<small>百万欧元</small><strong>${money(contract.releaseClause||0)}</strong></label><label>队内角色<small>承诺</small><strong>${esc(contract.role||"轮换球员")}</strong></label><label>合同年限<small>年</small><strong>${contract.years||"-"} 年</strong></label></div></section><div class="data-note">接受后将立即完成转会。你在 ${esc(seller.name)} 的本季数据会写入职业档案，新俱乐部从今天起重新统计。</div></div><div class="modal-actions">${active?`<button class="btn btn-danger" id="reject-player-transfer-offer">拒绝报价</button><button class="btn" data-close-modal>稍后决定</button><button class="btn btn-primary" id="accept-player-transfer-offer">接受并转会</button>`:`<button class="btn btn-primary" data-close-modal>返回</button>`}</div></div></div>`;
    }
