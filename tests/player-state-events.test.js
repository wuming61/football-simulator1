...
source=source.replace(/\n  render\(\);\n\}\)\(\);\s*$/,`
  Object.assign(window.__test,{CLUBS,PLAYER_STATE_EVENTS,createSquad,ensurePlayerContract,ensurePlayerCareer,eligiblePlayerStateEvents,startPlayerStateEvent,startPlayerStory,playerStoryChoices,resolvePlayerStoryChoice,settlePerformanceResponse,effectiveMatchAttribute,renderPlayerCareerModal,setTestState:value=>{state=value;},setModal:value=>{modal=value;}});
})();
`);
...
assert.equal(career.temporaryAttributeBoosts.passing,4);assert.equal(career.temporaryBoostMatches,2);assert.equal(career.responseMatches,2);
assert.ok(api.effectiveMatchAttribute(controlled,"passing",match,"ours")>=beforeBoost+4,"the selected detailed boost must feed the match engine");
...
api.settlePerformanceResponse(career,6.8,12);assert.equal(career.temporaryBoostMatches,2,"short cameos must not consume a temporary-state match");api.settlePerformanceResponse(career,6.8,65);api.settlePerformanceResponse(career,6.9,70);assert.equal(career.temporaryBoostMatches,0);assert.deepEqual(Object.keys(career.temporaryAttributeBoosts),[],"temporary boosts must expire after their promised appearances");
