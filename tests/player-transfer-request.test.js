...
const moved=state.squad.find(item=>item.id==="controlled");assert.equal(state.clubId,acceptedOffer.toId);assert.equal(moved.club,acceptedOffer.toId);assert.equal(moved.contract.weeklyWage,acceptedOffer.contract.weeklyWage);assert.equal(moved.appearances,0,"new club seasonal stats must start at zero");assert.ok(moved.careerStats.some(row=>row.clubId===formerClub&&row.appearances===21&&row.goals===9&&row.assists===8),"old club segment must be retained");assert.ok(state.transferHistory.some(row=>row.id&&row.fromId===formerClub&&row.toId===acceptedOffer.toId),"accepted transfer must enter permanent history");
